...
class MainFragment : Fragment() , MovieAdapter.MovieClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //val adapter = MovieAdapter(movieList as MutableList<Movie>, this)
        rv_movies.adapter = adapter
        //----------------------------------------------------------

        // If exist any change on Recycler View's data, then update the view
        model.data.observe(viewLifecycleOwner, Observer { adapter.updateMovieList(it) })
